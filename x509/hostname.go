...
func (h HostnameError) Error() string {
	return "x509: certificate is not valid for " + h.Host
}
...
func (c *Certificate) checkHost(host string, flags CheckFlags) error {
	...
	defer C.free(chost)
	rv := C.X509_check_host(c.x509, (*C.char)(chost), C.size_t(len(host)),
		C.uint(flags), nil)
	if rv > 0 {
		return nil
	}
	if rv == 0 {
		return HostnameError{c, host}
	}
...
func (c *Certificate) checkEmail(email string, flags CheckFlags) error {
	...
	defer C.free(cemail)
	rv := C.X509_check_email(c.x509, (*C.char)(cemail), C.size_t(len(email)),
		C.uint(flags))
	if rv > 0 {
		return nil
	}
...
func (c *Certificate) checkIP(ip net.IP, flags CheckFlags) error {
	...
	rv := C.X509_check_ip(c.x509, (*C.uchar)(cip), C.size_t(len(ip)),
		C.uint(flags))
	if rv > 0 {
		return nil
	}
